...
CreateThread(function()
	exports["esx_society"]:registerSociety('bikedealer', TranslateCap('car_dealer'), 'society_bikedealer', 'society_bikedealer', 'society_bikedealer', {type = 'private'})
end)
...
function SQLVehiclesAndCategories()
	categories = MySQL.query.await('SELECT * FROM bike_categories')
	vehicles = MySQL.query.await('SELECT bike.*, bike_categories.label AS categoryLabel FROM bike JOIN bike_categories ON bike.category = bike_categories.name')

	for _, vehicle in pairs(vehicles) do
		vehiclesByModel[vehicle.model] = vehicle
	end

	TriggerClientEvent("esx_bikeshop:updateVehiclesAndCategories", -1, vehicles, categories, vehiclesByModel)
end
...
RegisterNetEvent("esx_bikeshop:getVehiclesAndCategories", function()
	TriggerClientEvent("esx_bikeshop:updateVehiclesAndCategories", source, vehicles, categories, vehiclesByModel)
end)

RegisterNetEvent('esx_bikeshop:setVehicleOwnedPlayerId')
AddEventHandler('esx_bikeshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)

	if xPlayer.job.name ~= 'bikedealer' or not xTarget then
		return
	end

	MySQL.scalar('SELECT id FROM bikedealer_vehicles WHERE vehicle = ?', {model},
	function(id)
		if not id then
			return
		end

		MySQL.update('DELETE FROM bikedealer_vehicles WHERE id = ?', {id},
		function(rowsChanged)
			if rowsChanged == 1 then
				...
				MySQL.insert('INSERT INTO bike_sold (client, model, plate, soldby, date) VALUES (?, ?, ?, ?, ?)', {xTarget.getName(), label, vehicleProps.plate, xPlayer.getName(), os.date('%Y-%m-%d %H:%M')})
			end
		end)
	end)
end)
...
ESX.RegisterServerCallback('esx_bikeshop:getSoldVehicles', function(source, cb)
	MySQL.query('SELECT client, model, plate, soldby, date FROM bike_sold ORDER BY DATE DESC', function(result)
		cb(result)
	end)
end)

RegisterNetEvent('esx_bikeshop:rentVehicle')
AddEventHandler('esx_bikeshop:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)
	...
	MySQL.single('SELECT id, price FROM bikedealer_vehicles WHERE vehicle = ?', {vehicle},
	function(result)
		if not result then
			return
		end

		MySQL.update('DELETE FROM bikedealer_vehicles WHERE id = ?', {result.id},
		function(rowsChanged)
			if rowsChanged ~= 1 then
				return
			end

			MySQL.insert('INSERT INTO rented_bike (vehicle, plate, player_name, base_price, rent_price, owner) VALUES (?, ?, ?, ?, ?, ?)', {vehicle, plate, xTarget.getName(), result.price, rentPrice, xTarget.identifier},
			function(id)
...
RegisterNetEvent('esx_bikeshop:getStockItem')
...
RegisterNetEvent('esx_bikeshop:putStockItems')
...
ESX.RegisterServerCallback('esx_bikeshop:buyVehicle', function(source, cb, model, plate)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_bikeshop:getCommercialVehicles', function(source, cb)
	MySQL.query('SELECT price, vehicle FROM bikedealer_vehicles ORDER BY vehicle ASC', function(result)
		cb(result)
	end)
end)

ESX.RegisterServerCallback('esx_bikeshop:buybikedealerVehicle', function(source, cb, model)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_bikedealer', function(account)
		...
		MySQL.insert('INSERT INTO bikedealer_vehicles (vehicle, price) VALUES (?, ?)', {model, modelPrice},
		function(rowsChanged)
			cb(true)
		end)
	end)
...
RegisterNetEvent('esx_bikeshop:returnProvider')
AddEventHandler('esx_bikeshop:returnProvider', function(vehicleModel)
	...
	MySQL.single('SELECT id, price FROM bikedealer_vehicles WHERE vehicle = ?', {vehicleModel},
	function(result)
		if not result then
		...
		function(rowsChanged)
			...
			TriggerEvent('esx_addonaccount:getSharedAccount', 'society_bikedealer', function(account)
				...
				xPlayer.showNotification(TranslateCap('bike_sold_for', vehicleLabel, ESX.Math.GroupDigits(price)))
			end)
		end)
	end)
end)
...
ESX.RegisterServerCallback('esx_bikeshop:getRentedVehicles', function(source, cb)
	MySQL.query('SELECT * FROM rented_bike ORDER BY player_name ASC', function(result)
		local vehicles = {}
...
ESX.RegisterServerCallback('esx_bikeshop:giveBackVehicle', function(source, cb, plate)
	MySQL.single('SELECT base_price, vehicle FROM rented_bike WHERE plate = ?', {plate},
	function(result)
		if not result then
			return cb(false)
		end

		MySQL.update('DELETE FROM rented_bike WHERE plate = ?', {plate},
		function()
			MySQL.insert('INSERT INTO bikedealer_vehicles (vehicle, price) VALUES (?, ?)', {result.vehicle, result.base_price})
...
ESX.RegisterServerCallback('esx_bikeshop:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'bikedealer' or not Config.EnablePlayerManagement then
		...
		MySQL.single('SELECT * FROM rented_bike WHERE plate = ?', {plate},
		function(result)
...
RegisterNetEvent('esx_bikeshop:setJobVehicleState')
...
function PayRent()
	...
	MySQL.query('SELECT rented_bike.owner, rented_bike.rent_price, rented_bike.plate, users.accounts FROM rented_bike LEFT JOIN users ON rented_bike.owner = users.identifier', {},
	function(rentals)
		local owners = {}
		for i = 1, #rentals do
		...
		if next(unrentals) then
			MySQL.prepare.await('DELETE FROM rented_bike WHERE owner = ? AND plate = ?', unrentals)
		end
